...
class WinnersListRap extends StatefulWidget {
  const WinnersListRap({Key? key}) : super(key: key);

  @override
  _WinnersListRapState createState() => _WinnersListRapState();
}

class _WinnersListRapState extends State<WinnersListRap> {
  ...
  _searchWinners() async {
    ...
    var result = await _service.searchWinner(
        search: _searchTextController.text, pagination: page, videoType: 1);
    setState(() {
      isLoading = false;
    });
    if (result != null) {
